...
DEFAULT_CONFIG = {  # 默认配置；config.yaml 为空时程序会使用这一份配置。
    ...
    "voice": {  # 语音或文字提醒相关配置。
        "mode": "print",  # 提醒模式：print、audio_file、system_tts 或 espeak。
        "audio_file": "sounds/remind.wav",  # audio_file 模式下播放的音频文件。
        "max_seconds": 20,  # 单次提醒最多播放多少秒，超时后自动回到检测。
        "remind_texts": [  # 可随机选择的提醒文案列表。
            "主人，你已经长时间坐着了，该起来活动一下了。",
        ],  # 提醒文案列表结束。
    },  # 提醒配置结束。
}  # 默认配置结束。
...
def load_config(path="config.yaml"):  # 定义读取配置文件的函数。
...
def speak(text, voice_cfg):  # 定义提醒函数，支持打印、音频文件和 espeak。
    ...
    if mode == "audio_file":  # 如果是播放音频文件模式。
        ...
        if not audio_file:  # 如果没有配置音频文件。
            return  # 没有可播放内容，直接返回。
        try:  # 尝试调用系统播放器。
            subprocess.run(["aplay", audio_file], check=False, timeout=max_seconds)  # 使用 aplay 播放 wav 音频。
...
def main():  # 定义程序主函数。
    ...
    reset_seconds = sitting_cfg.get("reset_seconds", 20)  # 获取离开坐姿后清零计时的阈值。
        ...
        if monitor_enabled and now - last_detect_time >= detect_interval:  # 如果监测开启且达到检测间隔。
            ...
            if stable_sitting:  # 如果稳定状态是坐姿。
                ...
                if seated_since is None:  # 如果刚开始稳定坐着。
                    ...
                    print("[STATE] Stable sitting detected. Start sitting timer.")  # 打印开始计时提示。
                non_seated_since = None  # 清空非坐姿开始时间。

                sitting_duration = now - seated_since  # 计算连续坐姿时长。
                if sitting_duration >= sit_limit_seconds and now - last_remind_time >= remind_cooldown_seconds:  # 判断是否需要提醒。
                    ...
                    last_remind_time = now  # 更新最近提醒时间。
            else:  # 如果稳定状态不是坐姿。
                stable_state = "not_sitting"  # 更新状态为 not_sitting。
                if non_seated_since is None:  # 如果刚进入非坐姿状态。
                    non_seated_since = now  # 记录非坐姿开始时间。
                if seated_since is not None and now - non_seated_since >= reset_seconds:  # 如果已经离开坐姿足够久。
                    print("[STATE] User left or stood up long enough. Reset sitting timer.")  # 打印重置提示。
                    seated_since = None  # 清空坐姿开始时间。
                    last_remind_time = 0  # 清空提醒时间。

        sitting_seconds = time.time() - seated_since if seated_since is not None else 0  # 计算当前久坐秒数。
