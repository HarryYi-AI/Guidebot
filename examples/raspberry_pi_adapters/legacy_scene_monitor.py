...
PROJECT_ROOT = PROJECT_DEMO / "09.AI_Big_Model"
...
def get_tonyi_key():
    ...
    if is_empty_key(api_key):
        raise RuntimeError(
            "请先在 /home/pi/project_demo/09.AI_Big_Model/API_KEY.py "
            "或本 adapter 旁的 local_config.py 中填写 TONYI_key。"
        )
    return api_key
...
def speak_feedback(text):
    try:
        ...
        print(f"[{now_text()}] 请先运行：python3 legacy_scene_monitor.py --test-speak")
        return False
...
def infer_scene_label(record):
    ...
    if any(word in text for word in ("明火", "火焰", "起火", "着火", "烟雾", "fire", "smoke")):
        return "fire"
    if any(word in text for word in ("摔倒", "倒地", "跌倒", "fall")):
        return "fall"
    if judge.get("abnormal"):
        return "abnormal"
    return "normal"


def to_guidebot_payload(record):
    judge = record.get("judge_result", {})
    return {
        "label": infer_scene_label(record),
        "summary": record.get("scene_description", ""),
        "abnormal": bool(judge.get("abnormal", False)),
        "risk_level": str(judge.get("risk_level", "none")),
        "reason": str(judge.get("reason", "")),
        "feedback": str(judge.get("feedback", "")),
        "confidence": 0.9 if judge.get("abnormal") else 0.75,
    }
...
def parse_args():
    ...
    parser.add_argument(
        "--guidebot-json",
        action="store_true",
        help="只巡检一次，并向 stdout 输出 Guidebot 可消费的 JSON。",
    )
...
def main():
    ensure_runtime_dirs()
    args = parse_args()
    if args.guidebot_json:
        args.once = True
        args.speak = False
        args.report_normal = True
        with contextlib.redirect_stdout(sys.stderr):
            record = run_once(args)
        print(json.dumps(to_guidebot_payload(record), ensure_ascii=False), flush=True)
        return
